...
func _update_ambient(escaped_strength: float, escaped_dir: Vector3) -> void:
	...
	var volume_db: float = AudioServer.get_bus_volume_db(ambient_bus_idx)
	AudioServer.set_bus_volume_db(ambient_bus_idx, lerpf(volume_db, ambience, ambient_volume_interpolation))

	
	# Avg escape direction -> pan
	ambient_dir = ambient_dir.lerp(escaped_dir, ambient_pan_interpolation)
	var target_pan: float = 0.0 if ambient_dir.is_zero_approx() else self.transform.basis.x.dot(ambient_dir.normalized())
